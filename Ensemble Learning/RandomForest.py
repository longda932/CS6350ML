...
def buildTree(examples,r_features,node,f,level,n_f):
    s_labels=getColumn(examples,-1)
    if(len(set(s_labels))==1):
        node.addLabel(s_labels[0])
        node.addLeaf()
        return
    if(len(set(s_labels))!=1 and not r_features): 
    ...
    if(n_f<len(r_features)):
        # print("HERE")
        s_features=random.sample(r_features,n_f)
    else:
        s_features=r_features
    ...
    new_features=copy.copy(r_features)
    ...
    for key in dict:
        if(len(dict[key])==0):
            ...
            node.addChild(anode)
            buildTree(dict[key],new_features,anode,f,level,n_f)
...
# def random_forest(T,set):
#     nodeList=list()
#     n_samples=len(set)
#     for i in range(T):
#         root=treeNode(children=list())
#         temp_train_set=resample(set,n_samples=n_samples,replace=1)
#         buildTree(temp_train_set,features,root,'H',16)
...
# def learn_bagged_tree(T,nodeList):
...
# temp4=f_pres-f_pres.mean(axis=1)
...
# f_pres=np.zeros(len(test_set))
# for j in range(len(test_set)):
#     l1=0
#     l2=0
#     for i in range(T1):
#         if(pres[j,i]==labels[0]):
#             l1=l1+1
#     f_pres[j]=l1/T1
# true_table=np.zeros(len(test_set))
...
def forester_predict(T,nodeList):
...
def random_forest():
    ...
    single_list=list()
    for i in range(T):
        root1=treeNode(children=list())
        root2=treeNode(children=list())
        root3=treeNode(children=list())
        
        buildTree(train_set,features,root1,'H',20,2)
        buildTree(train_set,features,root2,'H',20,4)
        buildTree(train_set,features,root3,'H',20,6)
        if(i==0):
            single_list.append(root1)
            single_list.append(root2)
            single_list.append(root3)
        node_list[0].append(root1)
        node_list[1].append(root2)
        node_list[2].append(root3)

    return node_list,single_list

def single_random_forest():
    node_list=list()
    for i in range(T):
        root1=treeNode(children=list())
        

        buildTree(train_set,features,root1,'H',20,4)
        node_list.append(root1)

    return node_list


node_list,single_list=random_forest()
    


train_errors1,test_errors1=forester_predict(T,node_list[0])
train_errors2,test_errors2=forester_predict(T,node_list[1])
train_errors3,test_errors3=forester_predict(T,node_list[2])




fig=plt.figure(1)
plt.plot(range(T),train_errors1,'rv',label="training errors, size of feature:2")
plt.plot(range(T),test_errors1,color='red',label="testing errors, size of feature:2")
plt.plot(range(T),train_errors2,'gv',label="training errors, size of feature:4")
plt.plot(range(T),test_errors2,color='green',label="testing errors, size of feature:4")
plt.plot(range(T),train_errors3,'bv',label="training errors, size of feature:6")
plt.plot(range(T),test_errors3,color='blue',label="testing errors, size of feature:6")
temp_name="/Users/dalong/CS6350ML/Ensemble Learning/figure1.png"
fig.savefig(temp_name)
plt.legend()
plt.show()

print(train_errors1[-1])
print(train_errors2[-1])
print(train_errors3[-1])

print(test_errors1[-1])
print(test_errors2[-1])
print(test_errors3[-1])
