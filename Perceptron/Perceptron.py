...
def prediction_error(set,weights):
    ...
    for i in range(n_examples):
        if(torch.mm(weights.t(),set[i,:-1].view(-1,1))>0 and set[i,-1]>0):
            right=right+1
        elif(torch.mm(weights.t(),set[i,:-1].view(-1,1))<=0 and set[i,-1]==0.0):
            right=right+1
        else:
            wrong=wrong+1
...
def voted_prediction_error(set,t_c,t_dict,m):
    ...
    wrong=0
    for i in range(n_examples):
        if(voted_pred_helper(c,dict,set[i,:-1].view(-1,1))>0 and set[i,-1]>0):
            # print("HERE")
            right=right+1
        elif(voted_pred_helper(c,dict,set[i,:-1].view(-1,1))<=0 and set[i,-1]==0.0):
            # print("TERE")
            right=right+1
        else:
            wrong=wrong+1
...
def averaged_prediction_error(set,t_dict,m):
    ...
    wrong=0
    for i in range(n_examples):
        if(averaged_pred_helper(dict,set[i,:-1].view(-1,1))>0 and set[i,-1]>0):
            right=right+1
        elif(averaged_pred_helper(dict,set[i,:-1].view(-1,1))<=0 and set[i,-1]==0.0):
            right=right+1
        else:
            wrong=wrong+1
    return (wrong/(right+wrong))
